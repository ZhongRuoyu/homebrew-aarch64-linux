...
HOMEBREW_AARCH64_LINUX_BOOTSTRAP_URL="https://raw.githubusercontent.com/ZhongRuoyu/homebrew-portable-ruby-aarch64-linux/HEAD/bootstrap.sh"
...
then
  ...
  # On Linux, support 64-bit Intel and 64-bit ARM
  if [[ "${UNAME_MACHINE}" != "x86_64" ]] && [[ "${UNAME_MACHINE}" != "aarch64" ]]
  then
    abort "Homebrew on Linux is only supported on Intel and 64-bit ARM processors!"
  fi
fi
...
(
  ...
  if [[ -n "${HOMEBREW_ON_LINUX-}" ]] && [[ "${UNAME_MACHINE}" == "aarch64" ]]
  then
    ohai "Bootstrapping Homebrew on 64-bit ARM Linux..."
    HOMEBREW_AARCH64_LINUX_BOOTSTRAP_SCRIPT="$(
      execute "${USABLE_CURL}" -fsSL "${HOMEBREW_AARCH64_LINUX_BOOTSTRAP_URL}"
    )"
    execute /usr/bin/env "HOMEBREW_PREFIX=${HOMEBREW_PREFIX}" \
      /bin/bash <<<"${HOMEBREW_AARCH64_LINUX_BOOTSTRAP_SCRIPT}"
  fi
